...
def process_chat_message(user_message: str, trip_data: dict, chat_history: list, current_params: dict) -> dict:
    ...
    system_prompt = f"""You are a travel assistant that helps users plan their trip through natural conversation. Your job is to understand what users want and call the appropriate functions when needed.

Current trip context:
{json.dumps(context, indent=2)}

Available functions:
- search_flights: When user wants to search for new flights (different dates, different origin/destination)
- search_hotels: When user wants to search for new hotels (different area, different price range)  
- adjust_budget: When user wants to change their total budget
- change_strategy: When user wants to prioritize flights over hotels or vice versa
- adjust_iterations: When user wants more or fewer optimization attempts
- update_dates: When user wants to change travel dates
- update_preferences: When user wants to enable/disable red-eye flights
- find_better_hotel: When user explicitly asks to find a better/nicer/different hotel
- search_events: When user wants to find local events, things to do, concerts, festivals, food events, sports, etc.

CONVERSATION GUIDELINES:
1. Be natural and conversational - don't force function calls
2. If the user is just chatting or asking questions, don't call any functions
3. Only call functions when the user explicitly wants to take action
4. You can call multiple functions if the user asks for multiple things

UNDERSTANDING USER INTENT:
- "Find food events" → call search_events with preferences=["food"]
- "Find music events" → call search_events with preferences=["music"]
- "Find things to do" → call search_events with empty preferences
- "Find a better hotel" → call find_better_hotel
- "Find a cheaper hotel" → this is a hotel search with budget constraint, use search_hotels
- "Change my dates" → call update_dates
- "Increase my budget" → call adjust_budget

EXAMPLES OF NATURAL PHRASES:
- Events: "What's happening in NYC?", "Any concerts?", "Find food festivals", "Things to do on Tuesday"
- Hotels: "Find me a nicer hotel", "Show me cheaper options", "Hotels near downtown"
- Flights: "Look for earlier flights", "Any red-eye options?", "Change to morning flights"
- Budget: "I can spend more", "Reduce my budget", "Increase to $3000"

Remember: You're having a natural conversation. Only call functions when the user wants to take action. If they're just asking questions, respond conversationally using the context provided."""
        ...
        if user_message and (current_time - st.session_state.last_chat_time) >= RATE_LIMIT_SECONDS:
            ...
            with st.spinner("🤔 Thinking..."):
                ...
                if process_result["type"] == "function_calls" and process_result["function_calls"]:
                    ...
                    # CRITICAL FIX: Determine replanning from function calls, NOT from exec_result
                    should_replan = False
                    find_better = False
                    current_hotel_data = None
                    
                    # Check each function call to determine if replanning is needed
                    for func_call in process_result["function_calls"]:
                        func_name = func_call["function"]
                        
                        # ONLY these functions should trigger replanning
                        if func_name in ["search_flights", "search_hotels", "adjust_budget", 
                                        "change_strategy", "adjust_iterations", "update_dates", 
                                        "update_preferences", "find_better_hotel"]:
                            should_replan = True
                            print(f"🔴 REPLAN TRIGGERED by: {func_name}")  # Debug
                            
                        # Track find_better separately
                        if func_name == "find_better_hotel":
                            find_better = True
                            current_hotel_data = st.session_state.trip_result.get("data", {}).get("hotel", {})
                    
                    # Update params from exec_result (but only if they exist)
                    if exec_result["updates"]:
                        for key, value in exec_result["updates"].items():
                            st.session_state.current_params[key] = value
                    
                    # DEBUG: Print what's happening
                    print(f"📊 Function calls: {[fc['function'] for fc in process_result['function_calls']]}")
                    print(f"📊 should_replan: {should_replan}")

                # Step 3: Replan if needed (AND ONLY if should_replan is True)
                if should_replan:
